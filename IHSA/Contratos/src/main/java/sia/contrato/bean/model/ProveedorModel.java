...
public class ProveedorModel implements Serializable {
    ...
    public void procesarProveedor() {
        if (getProveedor().getLstDocsProveedor().size() > 0) {
    ...
    public void quitarArchivoDocumento(int idDocProv) {
        try {
            this.setDocProveedor(new ProveedorDocumentoVO());
            this.getDocProveedor().setId(idDocProv);
            this.eliminarArchivoDocumento();
            if (getContr() != null && getContr().getId() == -999) {
    ...
    public void quitarSoloArchivoDocumento(int idDocProv, int idDocProvAdj) {
        try {
            ...
            this.quitarArchivo();
            if (getContr() != null && getContr().getId() == -999
                    && contratoBean.getLstConveniosTabs().isEmpty()) {
                this.traerArchivosProveedor();
            } else {
                contratoBean.actualizarDocumentosProveedor();
            }
    ...
    public void agregarArchivoDocto(int idDocProv) {
        try {
            contratoBean.setSubirProveedor(true);
            this.setDocProveedor(new ProveedorDocumentoVO());
            this.getDocProveedor().setId(idDocProv);
            this.setDocProveedor(this.buscarDoctosConvePorId());
            contratoBean.setDocProveedor(this.getDocProveedor());
            String metodo = "";
