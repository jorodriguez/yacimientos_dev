...
@ViewScoped
public class IncidenciaConvenioBean implements Serializable {

    /**
     * Creates a new instance of IncidenciaBean
     */
    public IncidenciaConvenioBean() {
    ...
    private void llenarIncidencias() {
        setIncidencias(incidenciaImpl.traerPorUsuario(sesion.getUsuarioSesion().getId(), TicketEstadoEnum.NUEVO.getId()));
        getIncidencias().addAll(incidenciaImpl.traerPorUsuario(sesion.getUsuarioSesion().getId(), TicketEstadoEnum.ASIGNADO.getId()));
    }
    ...
    public void eliminarAAdjunto() {
        //
        int index = Integer.parseInt(FacesUtils.getRequestParam("indexAdj"));
        //
        adjuntoImpl.eliminarArchivo(incidenciasAdjunto.get(index).getId(), sesion.getUsuarioSesion().getId());
        ...
        try {
            ...
            if (addArchivo) {
                ...
                SiAdjunto adj = adjuntoImpl.save(documentoAnexo.getNombreBase(),
                        ...
                        fileUpload.getContentType(), fileUpload.getSize(), sesion.getUsuarioSesion().getId());
                //
                incidenciaAdjuntoImpl.agregarArchivoIncidencia(incidenciaVo.getIdIncidencia(), sesion.getUsuarioSesion().getId(), adj.getId());
                //
            ...
            Logger.getLogger(IncidenciaConvenioBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public void eliminarArchivo(int idIncAdj) {
        //
        incidenciaAdjuntoImpl.eliminarRelacion(idIncAdj, sesion.getUsuarioSesion().getId());
        incidenciasAdjunto = incidenciaAdjuntoImpl.traerArchivoPorIncidencia(incidenciaVo.getIdIncidencia());
    }

    public String uploadDirectoryTicket() {
        return new StringBuilder().append("Ticket/").append(sesion.getUsuarioSesion().getId()).toString();
    }
    ...
    private void ejecutarRegistro() {
        ...
        getIncidenciaVo().setIdCampo(sesion.getUsuarioSesion().getIdCampo());
        getIncidenciaVo().setIdGerencia(sesion.getUsuarioSesion().getGerencia() != null ? sesion.getUsuarioSesion().getIdGerencia() : Constantes.GERENCIA_ID_SGL);
//
        Usuario usuario = new Usuario();
        usuario.setId(sesion.getUsuarioSesion().getId());
        usuario.setNombre(sesion.getUsuarioSesion().getNombre());
        usuario.setEmail(sesion.getUsuarioSesion().getMail());
