...
//        html.append("<article>");
    ...
    //jevazquez 16/04/15
    protected String getTituloOficios(String titulo) {
	StringBuilder html = new StringBuilder();
	html.delete(0, this.cuerpoCorreo.length());
//        html.append("<article>");
	html.append("<header>");
	if (!titulo.equals(Constantes.OFICIOS_CORREO_ASUNTO_IHSA_PEMEX_NO_PROMOVIDOS + Constantes.FMT_ddMMyyy.format(new Date()))) {
	    html.append("<h3 style= \"text-decoration:none; border-bottom: 1px dotted #b5b5b5; color: #004181;padding:0px 12px 0px 12px;\"> ");
	    html.append(titulo);
	    html.append("</h3>");
	} else {
	    html.append("<h5 style= \"text-decoration:none; border-bottom: 1px dotted #b5b5b5; color: #004181;padding:0px 12px 0px 12px;\"> ");
	    html.append(titulo);
	    html.append("</h5>");
	}
	html.append("</header>");
