 ...
 * @author mluis
 */
@Named(value = "aprobarOrdenBean")
@ViewScoped
@Slf4j
public class AprobarOrdenBean implements Serializable {
    ...
    public void iniciar() {
        ...
        listaRevisa = ocFlujoImpl.getUsuariosPorAccion(Constantes.OCFLUJO_ACTION_APROBAR, usuarioVO.getIdCampo(), Constantes.NO_ELIMINADO);
        listaCampoUsuario();
        traerUsuarios();
    }
    ...
    public void agregarAprobadorOrdenCompra() {
        PrimeFaces.current().executeScript(";$(dialogoAprobarOcs).modal('show');");
    }
    ...
    public void guardarUsuarioApruebaOC() {
        if (!getUsuarioVO().getNombre().trim().isEmpty()) {
            ...
            if (usVo != null) {
                usuarioImpl.aprobarOrdenCompra(sesion.getUsuario().getId(), usVo.getId(), usuarioVO.getIdCampo(), Constantes.OCFLUJO_ACTION_APROBAR);
                listaRevisa = ocFlujoImpl.getUsuariosPorAccion(Constantes.OCFLUJO_ACTION_APROBAR, usuarioVO.getIdCampo(), Constantes.NO_ELIMINADO);
                usuarioVO.setNombre("");
