...
public class VistoBuenoCostosBean implements Serializable {
    ...
    /**
     * Creates a new instance of VistoBuenoCostosBean
     */
    public VistoBuenoCostosBean() {
    }
    ...
    public void requisicionesSinVistoBueno() {
        try {
            if (usuarioBean.getMapaRoles().containsKey("Visto Bueno Costos")) {
                listaRequisiciones = new ListDataModel(requisicionRemoto.requisicionesSinVistoBueno(usuarioBean.getUsuarioConectado().getId(),
                        usuarioBean.getUsuarioConectado().getApCampo().getId(), TipoRequisicion.PS.name(), Constantes.ROL_VISTO_BUENO_COSTO));
                List<OcUsoCFDI> lista = ocUsoCFDIImpl.traerCFDIPorTipo(TipoRequisicion.PS.toString());
