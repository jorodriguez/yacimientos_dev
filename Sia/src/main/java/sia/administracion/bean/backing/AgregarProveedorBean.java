 ...
 * @author mluis
 */
@ManagedBean(name = "agregarProveedorBean")
@ViewScoped
public class AgregarProveedorBean implements Serializable {
    ...
    public void subirArchivo(FileEntryEvent fileEntryEvent) {
        ...
        try {
            for (FileEntryResults.FileInfo fileInfo : fileEntryResults.getFiles()) {
                proveedorImpl.guardarProveedorDesdeArchivo(fileInfo.getFile(), usuarioBean.getUsuarioVO().getId());
                //
                listaProveedor = proveedorImpl.traerProveedorEstatus(usuarioBean.getUsuarioVO().getId(), ProveedorEnum.ACTIVO.getId(), 80);
