 ...
 * @author jcarranza
 */
@ManagedBean(name = "presupuestoAdminBean")
@ViewScoped
public class PresupuestoAdminBean implements Serializable {

    @ManagedProperty(value = "#{presupuestoAdminBeanModel}")
...
//            this.presupuestoAdminBeanModel.setPartidaProy(this.getNuevasActsPbjs().get(indice));
    ...
    public void uploadFile(FileEntryEvent fileEntryEvent) {
        ...
        try {
            ...
            for (FileEntryResults.FileInfo fileInfo : fileEntryResults.getFiles()) {
                try {
                    guardarArchivoPresupuesto(fileInfo);
                    completo = this.presupuestoAdminBeanModel.cargarPresupuestoFile(fileInfo.getFile());
                    if (!completo) {
