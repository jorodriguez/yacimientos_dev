...
public class CategoriaBean implements Serializable {
    ...
    @ManagedProperty(value = "#{categoriaModel}")
    ...
    */
    ...
    public void usuarioListener(TextChangeEvent event) {
        categoriaModel.setListaUsuario(event.getNewValue().toString());
    }
    ...
    public void traerArticulosItemsListenerSat(TextChangeEvent event) {
        String cadena = event.getNewValue().toString();
        if ((cadena != null && !cadena.isEmpty() && cadena.length() > 2)
                || (cadena == null || cadena.isEmpty())) {
