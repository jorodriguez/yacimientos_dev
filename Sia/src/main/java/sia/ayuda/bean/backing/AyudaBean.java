...
public class AyudaBean implements Serializable {
    ...
    @ManagedProperty(value = "#{ayudaBeanModel}")
...
//        return "/vistas/administracion/ayuda/catalogoAyudas";
    ...
    public void crearAyuda() throws Exception {
        ...
        if (ayudaBeanModel.getNombreAyuda() != null && !ayudaBeanModel.getNombreAyuda().equals("")) {
            try {
                ...
                cancelarCrearAyuda(actionEvent);
            } catch (Exception e) {
    ...
    public void actualizarAyuda() {
        try {
            if (getAyudaBeanModel().getNombreAyuda() != null && !ayudaBeanModel.getNombreAyuda().equals("") && !ayudaBeanModel.getNombreAyuda().equals("-1")) {
                ...
                cancelarActualizarAyuda(actionEvent);
            } else {
                FacesUtils.addInfoMessage("El nombre de la ayuda no puede ser vacío");
            }
        } catch (Exception e) {
    ...
    public void eliminarAyuda() {
        try {
            ...
            cancelarEliminarAyuda(actionEvent);
    ...
    public void subirArchivoAyudaPDF(FileEntryEvent fileEntryEvent) {
        ...
        try {
            ...
            for (FileEntryResults.FileInfo fileInfo : results.getFiles()) {
                ...
                if (fileInfo.getContentType().equals("application/pdf")) { //PDF
                    if (fileInfo.isSaved()) {
                        try {
                            getAyudaBeanModel().guardarArchivoAyudaPDF(fileInfo.getFileName(), fileInfo.getContentType(), fileInfo.getSize());
                        } catch (SIAException siae) {
                            FacesUtils.addErrorMessage(siae.getMessage());
                    ...
                    getAyudaBeanModel().eliminarArchivoFisicamente("Ayuda/" + getAyudaBeanModel().getAyudaSeleccionada().getId() + "/pdf/" + fileInfo.getFileName());
                }
            }
        } catch (Exception e) {
            e.getMessage();
    ...
    public void subirArchivoAyudaWMV(FileEntryEvent fileEntryEvent) {
